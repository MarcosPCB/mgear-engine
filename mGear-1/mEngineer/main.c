...
static void PannelLeft()
{
	...
	if(meng.pannel_choice==ADD_LIGHT)
	{
		...
		if(CheckColisionMouse(455,2710+810,810,455,0))
		{
			...
			DrawStringUI("Set the lightmap resolution",8192,8192-455,0,0,0,255,128,32,255,st.fonts[ARIAL].font,2048,2048,0);
		...
		if(meng.command==LIGHTMAP_RES)
		{
			...
			{
				DrawStringUI(str,8192,4096-512,0,0,0,255,128,32,255,st.fonts[ARIAL].font,2048,2048,0);
				...
				DrawStringUI(str,8192,4096-512,810,227,0,255,255,255,255,st.fonts[ARIAL].font,2048,2048,0);

			if(meng.sub_com==1)
			{
				meng.lightmap_res.x=atoi(st.TextInput);

				DrawStringUI(str,8192,4096-512,810,227,0,255,32,32,255,st.fonts[ARIAL].font,2048,2048,0);
			...
			if(CheckColisionMouse(8192,4096+512,810,455,0))
			{
				DrawStringUI(str,8192,4096+512,810,227,0,255,128,32,255,st.fonts[ARIAL].font,2048,2048,0);
	...
	if(meng.command2==EDIT_SPRITE)
	{
			...
			DrawString2(str,p.x,p.y-227,810,217,0,255,255,255,255,st.fonts[ARIAL].font,p2.x,p2.y,0);
			...
			DrawString2(str,p.x-455,p.y,810,217,0,255,255,255,255,st.fonts[ARIAL].font,p2.x,p2.y,0);
...
static void ViewPortCommands()
{
	...
	if(!CheckColisionMouse(455,4096,910,8192,0) && meng.command!=MGG_SEL)
	{
				...
				for(i=0;i<st.Current_Map.num_sprites;i++)
				{
					...
					if(CheckColisionMouseWorld(st.Current_Map.sprites[i].position.x,st.Current_Map.sprites[i].position.y,st.Current_Map.sprites[i].body.size.x,st.Current_Map.sprites[i].body.size.y,st.Current_Map.sprites[i].angle))
					{
						if(st.mouse1)
						{
							...
							DrawString2(str,p.x+227,p.y-227,405,217,0,255,255,255,255,st.fonts[ARIAL].font,1024,1024,0);
		...
		if(meng.pannel_choice==ADD_LIGHT && meng.command==EDIT_LIGHTMAP2)
		{
			...
			if(!meng.sub_com)
			{
				...
				else
				{
					...
					if(CheckColisionMouseWorld(p.x,p.y,455,455,0))
					{
						DrawString2("Done Light",p.x,p.y,0,0,0,255,128,32,255,st.fonts[ARIAL].font,1536,1536,0);
						...
						DrawString2("Done Light",p.x,p.y,0,0,0,255,255,255,255,st.fonts[ARIAL].font,1536,1536,0);
				}
		...
		if(meng.pannel_choice==ADD_LIGHT && meng.command==RGB_LIGHTMAP)
		{
			...
			{
				DrawStringUI("Done",8192,4096+(341*3),0,0,0,255,128,32,255,st.fonts[ARIAL].font,2048,2048,0);
				...
				DrawStringUI("Done",8192,4096+(341*3),0,0,0,255,255,255,255,st.fonts[ARIAL].font,2048,2048,0);
		}
		else
