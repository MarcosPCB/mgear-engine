...
int8 InitMGLCode(const char *file)
{
	...
	st.mgl.val = st.mgl.tmp = st.mgl.v1 = st.mgl.v2 = st.mgl.v3 = st.mgl.v4 = st.mgl.i = st.mgl.bp = st.mgl.sp = 0;

	return 1;
}

#define PushStack(x) { stack[sp] = x >> 24; stack[sp + 1] = (x >> 16) & 0xFF; stack[sp + 2] = (x >> 8) & 0xFF; stack[sp + 3] = x & 0xFF; sp += 4; }
#define PopStack(x) { x = (stack[sp - 4] << 24) | (stack[sp - 3] << 16) | (stack[sp - 2] << 8) | stack[sp - 1]; sp -= 4; }

#define GetVarAddress(x) { tmp = (buf[x + 1] << 24) | (buf[x + 2] << 16) | (buf[x + 3] << 8) | buf[x + 4]; if (buf[x] == 1) tmp += bp; }
#define GetValueCV(x, loc) { x = (buf[loc] << 24) | (buf[loc + 1] << 16) | (buf[loc + 2] << 8) | buf[loc + 3]; }
#define GetValueStack(x, loc) { x = (stack[loc] << 24) | (stack[loc + 1] << 16) | (stack[loc + 2] << 8) | stack[loc + 3]; }
#define CodeToStack(loc, x) { stack[loc] = buf[x]; stack[loc + 1] = buf[x + 1]; stack[loc + 2] = buf[x + 2]; stack[loc + 3] = buf[x + 3]; }
#define StackToStack(loc, loc2) { stack[loc] = stack[loc2]; stack[loc + 1] = stack[loc2 + 1]; stack[loc + 2] = stack[loc2 + 2]; stack[loc + 3] = stack[loc2 + 3]; }
#define SetStack(x, loc) { stack[loc] = x >> 24; stack[loc + 1] = (x >> 16) & 0xFF; stack[loc + 2] = (x >> 8) & 0xFF; stack[loc + 3] = x & 0xFF; }

///These are the switch cases for the registers
#define SetRegval case 12: val = cur; break;
#define SetRegtmp case 13: tmp = cur; break;
#define SetRegv1 case 14: v1 = cur; break;
#define SetRegv2 case 15: v2 = cur; break;
#define SetRegv3 case 16: v3 = cur; break;
#define SetRegv4 case 17: v4 = cur; break;
#define SetRegi case 19: i = cur; break;
#define SetRegbp case 20: bp = cur; break;
#define SetRegsp case 21: sp = cur; break;
#define SetRegcv case 22: cv = cur; break;

#define GetRegval case 12: cur = val; break;
#define GetRegtmp case 13: cur = tmp; break;
#define GetRegv1 case 14: cur = v1; break;
#define GetRegv2 case 15: cur = v2; break;
#define GetRegv3 case 16: cur = v3; break;
#define GetRegv4 case 17: cur = v4; break;
#define GetRegi case 19: cur = i; break;
#define GetRegbp case 20: cur = bp; break;
#define GetRegsp case 21: cur = sp; break;
#define GetRegcv case 22: cur = cv; break;

#define GetRegf2 case 1: f1 = f2; break; 
#define GetRegf3 case 2: f1 = f3; break; 
#define GetRegf4 case 3: f1 = f4; break; 

#define SetRegf2 case 1: f2 = f1; break; 
#define SetRegf3 case 2: f3 = f1; break; 
#define SetRegf4 case 3: f4 = f1; break; 

#define GetRegSwitch(x) switch(x) { GetRegval; GetRegtmp; GetRegv1; GetRegv2; GetRegv3; GetRegv4; GetRegi; GetRegbp; GetRegsp; GetRegcv; }
#define SetRegSwitch(x) switch(x) { SetRegval; SetRegtmp; SetRegv1; SetRegv2; SetRegv3; SetRegv4; SetRegi; SetRegbp; SetRegsp; SetRegcv; }

#define GetRegfSwitch(x) switch(x) { GetRegf2; GetRegf3; GetRegf4; }
#define SetRegfSwitch(x) switch(x) { SetRegf2; SetRegf3; SetRegf4; }

int8 ExecuteMGLCode(uint8 location)
{
	register int32 val, tmp, v1, v2, v3, v4, i, cur, bp, sp, cv;

	static char stack1[131072], *stack2, *stack;
	uint8 *vars;
	void **heap;
	...
	233 - compstring string string - val
	234 - copystring string string
	235 - tokenstring string string - val
	236 - stringstring string string  - val
	237 - charstring string x - val

	238 - openfile buffer string - val
	239 - closefile buffer
	240 - printfile buffer string ...
	241 - scanfile buffer string ...
	242 - writefile buffer buffer x - val
	243 - readfile buffer buffer x - val
	244 - seekfile buffer x y - val
	245 - rewindfile buffer
	246 - sizefile buffer - val
	...
	*/
	...
	if (location == 0)
	{
		...
		while (buf[cv] == 254)
		{
			...
			if (buf[cv + 1] == 4)
			{
				cur += (buf[cv + 2] << 24) | (buf[cv + 3] << 16) | (buf[cv + 4] << 8) | buf[cv + 5];
				cv += 6;

				for (cur += bp; bp < cur; bp++, cv++)
					stack[bp] = buf[cv];
	...
	while (bp > st.mgl.ret_addr)
	{
		...
		else
		{
			switch (buf[cv])
			{
				...
				case 66:
					GetVarAddress(cv + 1);
					GetValueStack(f1, tmp);
					GetValueCV(f2, cv + 5);
					f1 = pow(f1, f2);
					memcpy(stack + tmp, &f1, 4);
					//Turn to big endian
					cur = stack[tmp];
					val = stack[tmp + 1];
					stack[tmp] = stack[tmp + 3];
					stack[tmp + 1] = stack[tmp + 2];
					stack[tmp + 2] = val;
					stack[tmp + 3] = cur;
					cv += 10;
					break;
