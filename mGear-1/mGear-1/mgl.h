...
#define MAX_STACKMEM MAX_VARS*4 //32-bit variables
#define MAX_FUNCTIONS 512
#define MAX_ARRAYMEM 512

//In the compiled MGMUL file the first 8 bytes are NULL
...
//jump 4 bytes and then load the function addresses
//each function is represented by a 16-bit address
//so you must load all the entries. Ex: 64 functions - 64 * 2 bytes = 128 bytes of function address
//jump 4 bytes after the addresses and the next 4 bytes represents the total size of the MGMUL code
...
struct _MGL_SYS
{
	...
	int32 *STACK; //Stack memory

	int32 *bin_file; //compiled MGMUL file

	int16 *function_addresses;
};
