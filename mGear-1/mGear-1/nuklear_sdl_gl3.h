...
nk_sdl_device_create(int max_vertex_buffer, int max_element_buffer)
{
	...
	glBufferData(GL_ARRAY_BUFFER, max_vertex_buffer, NULL, GL_STATIC_DRAW);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, max_element_buffer, NULL, GL_STATIC_DRAW);
...
NK_API void
nk_sdl_render(enum nk_anti_aliasing AA, int max_vertex_buffer, int max_element_buffer, size_t buffer_size)
{
    struct nk_sdl_device *dev = &sdl.ogl;
    int width, height;
   ...
   // glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
    ...
    glUniformMatrix4fv(dev->uniform_proj, 1, GL_FALSE, &ortho[0][0]);
    {
		...
		vertices = calloc(max_vertex_buffer, 1);
		elements = calloc(max_element_buffer, 1);
		cmdsb = calloc(buffer_size, 1);
       ...
       // elements = glMapBuffer(GL_ELEMENT_ARRAY_BUFFER, GL_WRITE_ONLY);
        {
			...
			nk_buffer_init_fixed(&cmds, cmdsb, (nk_size) buffer_size);
            nk_buffer_init_fixed(&vbuf, vertices, (nk_size)max_vertex_buffer);
            nk_buffer_init_fixed(&ebuf, elements, (nk_size)max_element_buffer);
            nk_convert(&sdl.ctx, &cmds, &vbuf, &ebuf, &config);
       }
        ...
        nk_draw_foreach(cmd, &sdl.ctx, &cmds) {
	...
	//nk_buffer_free(sdl.ctx.draw_list.vertices);
...
nk_sdl_init(SDL_Window *win, size_t buffer_size, int max_vertex_buffer, int max_element_buffer)
{
    sdl.win = win;
